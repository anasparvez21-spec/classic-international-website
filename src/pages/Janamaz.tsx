...
const Janamaz: React.FC = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { addItem, openCart } = useCartStore();
  const products = getProductsByCategory('janamaz');
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4 relative">
      ...
      <h1 className="text-3xl font-bold mb-8 text-center">Janamaz</h1>
      ...
      {hoverImg && (
        <div className="fixed left-1/2 top-24 z-50 flex flex-col items-center" style={{transform: 'translateX(-50%)'}}>
          <div className="bg-white rounded-lg shadow-2xl border-4 border-gray-200 p-2 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] scale-95 opacity-0 animate-enlarge-fade-in">
            <img src={hoverImg} alt="Preview Janamaz" className="max-h-[60vh] max-w-[80vw] rounded-lg transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]" />
          </div>
        </div>
      )}
    </div>
