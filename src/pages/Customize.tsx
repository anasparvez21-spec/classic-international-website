...
const Customize: React.FC = () => {
  ...
  const products = getProductsByCategory('customization');
  console.log('Customization products:', products);
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4 relative">
      {products.length === 0 && (
        <div className="text-center text-red-600 font-bold mb-8">
          No customization products found. Check your product data or category values.
        </div>
      )}
      ...
      <h1 className="text-3xl font-bold mb-8 text-center">Customization Products</h1>
            ...
            >
              <img
                ...
                onError={(e) => {
                  ...
                  if (target.src.endsWith(product.featuredImage)) {
                    target.src = '/collection-accessories.jpg';
                  }
                }}
      ...
      {hoverImg && (
        <div className="fixed left-1/2 top-24 z-50 flex flex-col items-center" style={{transform: 'translateX(-50%)'}}>
          <div className="bg-white rounded-lg shadow-2xl border-4 border-gray-200 p-2 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] scale-95 opacity-0 animate-enlarge-fade-in">
            <img src={hoverImg} alt="Preview Customization" loading="lazy" decoding="async" className="max-h-[60vh] max-w-[80vw] rounded-lg transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]" />
          </div>
        </div>
      )}
    </div>
